...
jQuery(document).ready(function($) {
	...
	function SearchYouTube(query, loadmore, number) {
		...
		$.ajax({
			...
			url:"https://www.googleapis.com/youtube/v3/search?part=snippet&q=" + query + "&type=video&key=AIzaSyB3Bsb2jSqB3vzDOYqWL-Kl8MwGw0DPimc",
			dataType: 'json',
				...
				if(data.items) {
					...
					for (i = 0; i < data.items.length; i++) {
						...
						if(data.items[i].snippet != undefined) {
							...
							row += "<tr>";
							row += "<td class='youtube_image' vAlign='top' align='left'>";
							row += "<a href='#' ><img width='120px' height='80px' src=" + data.items[i].snippet.thumbnails.default.url + " /></a>";
							row += "</td>";
							row += "<td class='youtube_info' vAlign='top' width='100%' align='left'>";
							row += "<a href='#' class='youtube_title'><b>" + data.items[i].snippet.title + "</b></a><br />";
							row += "<span style='font-size:12px; color:#555555'>by auteur</span><br />";
							row += "<span style='font-size:12px' color:#666666> vue views <span><br />";
							row += "<span style='font-size:12px' color:#666666> duree<span><br />";
							row += "</td>";
							row += "<td class='youtube_url' vAlign='top' align='left' data-id='"+data.items[i].id.videoId+"' >";
							row += "<input class='youtube_video_url' type='hidden' value='http://www.youtube.com/watch?v="+data.items[i].id.videoId+"' /><br />";
							row += "</td>";
							row += "</tr>";
							row += "</table>";
							row += "</div>";
							...
							row += "Aucun résultat. Essayer une autre recherche SVP.";
						}
					}
					row += '<div class="load_more_results"><span class="get_more_results btn-default">Charger plus...</span></div>';
	...
	$('#youtube_insert').click(function() {
		...
		if(this_link == '') {
			...
			alert('Choisir une vidéo; ou ajouter un "lien YouTube".');
			return false;
		}
