$(document).ready( function () {
	...
	$('#ajouter_champs').on('click', function(){
		$.ambiance({message: 'Cette fonctionnalité sera bientôt disponible !',
	             type: "information", 
	             timeout: 10});
		return false;
	})
	...
	/***********************/
	$('.etats_date_livraison').on('change',function(){
		valeur = $('.etats_id_livreur').val();
        valeur2 = $('.etats_date_livraison').val();	

        

        $.ajax({type:"GET", data: 'date_livraison='+valeur2+'&id_livreur='+valeur , url:'admin_etats_livraisons.php', 
	...
	$('.etats_id_livreur').on('change',function(){
		valeur = $('.etats_id_livreur').val();
        valeur2 = $('.etats_date_livraison').val();	
	...
	$('#inputdate,#inputdate2').on('change',function(){
		valeur = $('#inputdate').val();
		valeur2 = $('#inputdate2').val();

		//console.log(valeur+'/'+valeur2);
		

        $.ajax({type:"POST", data: 'date='+valeur+'&date2='+valeur2 , url:'admin_system_dashboard.php', 
        	success: function(server_response){
          		$('#content').html(server_response);
            },	
            complete: function(data) {
	...
	/*********************/
	$('.date_livraison').on('change',function(){
		valeur = $(this).val();
		/*$.ajax({type:"GET", data: 'date_livraison='+valeur , url:'ajax_load_produits_a_livrer.php', 
        	success: function(server_response){
        ...
        $.ajax({type:"GET", data: 'date_livraison='+valeur , url:'ajax_load_zone_a_livrer.php', 
        	success: function(server_response){
          		$('#zone_livraison').html(server_response);
            },	
            complete: function(data) {
            	//$("#ajax-loading").hide();
            },                       
	...
	$('.id_livreur').on('change',function(){
		//alert($(this).val());
		valeur = $('.date_livraison').val();
		/*$.ajax({type:"GET", data: 'date_livraison='+valeur , url:'ajax_load_produits_a_livrer.php', 
        	success: function(server_response){
          		$('#container-commandes').html(server_response);
            },	
        ...
        $.ajax({type:"GET", data: 'date_livraison='+valeur+'&id_livreur='+$(this).val() , url:'ajax_load_produits_a_livrer.php', 
        	success: function(server_response){
          		$('#container-commandes').html(server_response);
            },	
            complete: function(data) {
	...
	$('.imprim_livreur').on('change',function(){
		alert(1);
		if($('.imprim_date_livraison').val() == 0){

		}
		alert($('.imprim_date_livraison').val());
	});

	/********************/


	$('.choix_ville').on('change',function(){
        valeur = $(this).val();
        if((valeur !== '44') && (valeur !== '853')){
            $('.choix_quartier').attr('disabled','disabled').hide().prev('label').hide();
        }else{
            $('.choix_quartier').removeAttr('disabled').show().prev('label').show();
        }
    });

    $('.choix_pays').on('change',function(){
        valeur = $(this).val();
        if((valeur !== '44')){
            $('.choix_quartier').attr('disabled','disabled').hide().prev('label').hide();
        }else{
    ...
    $('#rech_back_produit').on('keypress',function(e){
    	if(e.keyCode == 13){
    		$.ajax({type:"GET", data: 'rech='+$('#rech_back_produit').val() , url:'ajax_load_produits.php', 
	        	success: function(server_response){
	              	//if(server_response != ""){
	              		//$('#receiver_produit').html(server_response);
	              		$('#receiver_produit .back_produit').not('.choisi').remove();
	              		$('#receiver_produit').append(server_response);
	              	//}	              	
	            },	
	            complete: function(data) {
	            	//$("#ajax-loading").hide();
	...
	$('#phone').on('change', function(){
		phone = $(this).val();
		if((phone).length > 4){
			$.ajax({type:"GET", dataType: 'json', data: {'phone' : phone} , url:'ajax_load_boutique_user.php', 
	        	success: function(server_response){
	              	//$('#id_chapitre').html(server_response);
	              	if(server_response != ""){
	              		$('#id_client').val(server_response.id);
	              		$('#nom_client').val(server_response.nom);
	              		$('#prenoms_client').val(server_response.prenoms);
	              		$('#email').val(server_response.email);
	              		//$('#phone').val(server_response.phone);
	              		$('#pays').val(server_response.pays);
	              		$('#ville').val(server_response.ville);
	              		$('#quartier').val(server_response.quartier);
	              		$('#informations').val(server_response.informations);
	              		$('#sexe').val(server_response.sexe);
...
///////////////////////////////////////////////////////////	
	...
	$("#formActualites").submit(function(){
		if (checkVide("#inputlibelle","Erreur :<span class='noir'> Veillez svp remplir le champ </span> Libellé") || 
			checkVide("#inputdescription___","Erreur :<span class='noir'> Veillez svp remplir le champ </span> Description") /*|| 
			checkVide("#inputdate_pub","Erreur :<span class='noir'> Veillez svp remplir le champ </span> Date de Publication")*/) {
			return false;
		}else{
			retour = $(this).attr("data-retour");
