...
$medecins = array(0 => 'Choisir le médecin');
$medecins_tab = $Model->extraireTableau('*','users', 'valid= 1 AND statut = 1 AND type = 1');
if(!empty($medecins_tab)){
	foreach ($medecins_tab  as $k => $v) {
	   $medecins[$v['id']] = $v['nom'].' '.$v['prenoms'];
	}
}

$centres = array();
$centres_tab = $Model->extraireTableau('*','centres', 'valid= 1 AND statut = 1');
if(!empty($centres_tab)){
	foreach ($centres_tab  as $k => $v) {
	   $centres[$v['id']] = $v['name'];
	}
}
...
if(isset($_GET['update']) && empty($_GET['update'])) { 
	...
	if(!empty($_POST['id_centre']) && $_POST['id_medecin'] != 0){
		$datas = array();
		foreach($_POST['id_centre'] as $k => $v){
			$row = $_POST;
			$row['id_centre'] = $v;
		...
		$_POST['date_enreg'] = date('Y-m-d');	
		$_POST = $Model->checkTableFields($_POST,$table);
		$last_insert_id = $Model->insert($datas,$table);
		if($last_insert_id){		
		...
		$msg = "<span class ='noir'>Erreur :</span> Veuillez bien renseigner les champs ";
		flash($msg,"error",true,10);
	}
}
...
if(isset($_GET['edit']) && !empty($_GET['edit'])){ # Si le paramètre edit est spécifié dans l'URL
	...
	$centre_choisi = $Model->extraireChamp('id_centre','affectations_medecins', 'valid= 1 AND statut = 1 AND id = '.$id);		
	$Form->set($elements);
}
...
switch (isset($_GET['action'])?$_GET['action']:'') {
	case 'liste':
		$html = listeAffectationsMedecins();
		echo $html;
		break;

	case 'form':
		$html = FormAffectationsMedecins();
		echo $html;
		break;
