...
$experiences = array(1=>'Nourisson',2=>'Bambin',3=>'Maternelle',4=>'Ecole primaire',5=>'Adolescent');
...
if(isset($_GET['update']) && !empty($_GET['update'])) {
	...
	$_POST['experiences'] = implode('#',$_POST['experiences']);
...
if(isset($_GET['edit']) && !empty($_GET['edit'])){ # Si le paramètre edit est spécifié dans l'URL
	...
	$user_experiences = explode('#',$elements['experiences']);
}
...
$trier = array('id' => 'DESC');
$data = $Model->get('*',$table,$conditions,"AND",$trier,$limite = null);//var_dump($data);
...
switch (isset($_GET['action'])?$_GET['action']:'') {
	case 'liste':
		$html = listeBabysitters();
		echo $html;
		break;

	case 'form':
		$html = formBabysitters();
		echo $html;
		break;
