...
if(isset($_GET['action']) && $_GET['action'] == 'candidats'){
	$data = $Model->extraireTableau('users.*','participer_events LEFT JOIN users ON users.id=participer_events.id_user','users.valid=1 AND users.statut=1 AND id_event='.$_GET['id_event'].' AND users.type = 0');
	$current_event = $Model->extraireChamp('libelle_fr','articles','id='.$_GET['id_event']);
}elseif(isset($_GET['action']) && $_GET['action'] == 'recruteurs'){
	$data = $Model->extraireTableau('users.*,participer_events.besoin','participer_events LEFT JOIN users ON users.id=participer_events.id_user','users.valid=1 AND users.statut=1 AND id_event='.$_GET['id_event'].' AND users.type = 1');
	$current_event = $Model->extraireChamp('libelle_fr','articles','id='.$_GET['id_event']);
}else{
...
switch (isset($_GET['action'])?$_GET['action']:'') {
	...
	case 'candidats':
		$html = listeEventCandidats();
		echo $html;
		break;

	case 'recruteurs':
		$html = listeEventRecruteurs();
		echo $html;
		break;
